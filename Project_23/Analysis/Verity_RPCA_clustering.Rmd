...
FeaturePlot(Verity.combined,features=c("Aldh1l1","Gfap","Syp","Csf1r","Tmem119"), raster=FALSE) + ggtitle("Integrated Verity data, labelled with some gene markers", subtitle="RPCA,Clustering resolution 1, 30 PCs used")
...
Verity.combined <- IntegrateData(anchorset = Verity.anchors_CCA)

#save the object
#save(Verity.combined_CCA,"Rdata/Verity.self_integrated.Rdata")
...
Verity.combined_CCA <- RunUMAP(Verity.combined_CCA, reduction = "pca", dims = 1:30)
Verity.combined_CCA <- FindNeighbors(Verity.combined_CCA, reduction = "pca", dims = 1:30)
Verity.combined_CCA <- FindClusters(Verity.combined_CCA, resolution = 0.5)

# Visualization
p1CCA <- DimPlot(Verity.combined_CCA, reduction = "umap", group.by = "treatment")
p2CCA <- DimPlot(Verity.combined_CCA, reduction = "umap", label = TRUE, repel = TRUE)
p1CCA + p2CCA

#Visualise treatments side by side
DimPlot(Verity.combined_CCA, reduction = "umap", split.by = "treatment")
```
