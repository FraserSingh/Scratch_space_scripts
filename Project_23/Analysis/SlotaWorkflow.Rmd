...
# Clustering, this is all Slota only work, for crossover with Verity, move to Harmony.Rmd
...
p3 <- DimPlot(Slota.combined, reduction = "umap", group.by =  'integrated_snn_res.0.3', label = TRUE,repel = TRUE, raster=FALSE)
...
FeaturePlot(Slota.combined,features="Slc1a3", raster=F)+ggtitle("Integrated Verity data, labelled with astrocyte gene markers", subtitle="RPCA,Clustering resolution 0.2, 25 PCs used")
...
#Save the object
save(Slota.combined,file="Rdata/Slota.self_integrated_RPCA_annotated.Rdata")
```

## Investigate astrocytes

```{r}
load("Rdata/Slota.self_integrated_RPCA_annotated.Rdata")
...
Slo_astrocytes<-subset(Slota.combined, idents=c('5','12','19','10'))
...
Slo_astrocytes@assays$RNA@counts <-round(Slo_astrocytes@assays$RNA@counts)
...
save(astrocyte.markers, file="Slota_astrocyte.markers_Redux.RData")
```
...
Slota_astrocyte_prion.markers <- FindMarkers(Slo_astrocytes, ident.1 = 'RML_HP',ident.2 = 'PBS_HP', min.pct = 0.10, verbose = TRUE, test.use = "DESeq2", logfc.threshold = 0.25)
...
#Save the file
write.csv(Slota_astrocyte_prion.markers, file = "Slota_astrocyte_prion.markers_Redux.csv")

save(Slota_astrocyte_prion.markers, file="Slota_astrocyte_prion.markers_Redux.RData")

# Do the same for the infected Cortex samples and their control
Slota_astrocyte_prion.markers_CX <- FindMarkers(Slo_astrocytes, ident.1 = 'RML_CX',ident.2 = 'PBS_CX', min.pct = 0.10, verbose = TRUE, test.use = "DESeq2", logfc.threshold = 0.25)
...
#Save the file
write.csv(Slota_astrocyte_prion.markers_CX, file = "Slota_astrocyte_prion.markers_CX_Redux.csv")

save(Slota_astrocyte_prion.markers_CX, file="Slota_astrocyte_prion.markers_CX_Redux.RData")
