...
#Get annotation info
library("biomaRt")
m <- useMart("ensembl", dataset="mmusculus_gene_ensembl", version="")
```
...
#showing the missing NA entrez names
table(is.na(names(geneList_entrezname)))
table(duplicated(names(geneList_stdname)))
data.frame(names(geneList_stdname[is.na(names(geneList_entrezname))]))
#the lists are the same, so index positions are consistent across objects
length(geneList_entrezname)
length(geneList_stdname)
#show the index position of some of the NA genes in the symbols object
match('Zfp831', names(geneList_stdname))
match('Zfp846', names(geneList_stdname))
#show that the names are NA in the entrez list, with an example of an appropriately named entrez gene too
names(geneList_stdname)[1]
names(geneList_entrezname)[6485]
names(geneList_entrezname)[6484]
names(geneList_entrezname)[6486]

data.frame(geneList_stdname)
data.frame(geneList_entrezname)
