...
#Warning! some genes not mapping, see:
# View(Verity_DEGs_PrionUp__entrez[is.na(Verity_DEGs_PrionUp__entrez$ENTREZID),])
...
gene_info_2<-getBM(attributes = c("entrezgene_id"), filters = "external_gene_name", values = Verity_astrocyte_prion_markers$gene, mart = m)
names(geneList_entrezname)<-gene_info_2$entrezgene_id
...
#filter normal name df by the rows of the entrez name df which are duplicates/NA
non_mapped_ids<-geneList_stdname[duplicated(names(geneList_entrezname))]
as.character(
  (bitr(names(non_mapped_ids), fromType = "SYMBOL", toType = "ENTREZID", OrgDb = org.Mm.eg.db))$ENTREZID
)

geneList_entrezname[names(geneList_entrezname)=='330788']
...
ego3 <- gseGO(geneList     = geneList, #This is the DEG list full list of DEGs
              OrgDb        = org.Mm.eg.db,
              ont          = "ALL",
              minGSSize    = 100,
              maxGSSize    = 500,
