...
#Address the potential non-mapping gene entrezIDs, make a gene list with fewwer NAs

#make list of gene names not mapping

x<-(Verity_astrocyte_prion_markers[is.na(Verity_astrocyte_prion_markers$entrezgene_id),]$gene)
x<-x[!duplicated(x)]
...
#make a single column for everything recovered
b <- a %>%
  mutate(entrezgene_id = coalesce(ENTREZID, entrezgene_id))
b$entrezgene_id<-as.integer(b$entrezgene_id)
#remove old columns
b$ENTREZID<-ENTREZID<- NULL


c<-b[!is.na(b$entrezgene_id),] #The final list of newly gnerated entrezIDs

Verity_astrocyte_ageing_markers <- merge(Verity_astrocyte_prion_markers, c, by.x = "gene", by.y = "ALIAS", all.x = TRUE)
...
geneList <- Verity_astrocyte_ageing_markers_entrez$avg_log2FC
## feature 2: named vector of enembl IDs
names(geneList) <-Verity_astrocyte_ageing_markers_entrez$entrezgene_id
## feature 3: decreasing order
geneList <- sort(geneList, decreasing = TRUE) #object of type double

#Create upregulated significant genes
# Subset by Upregulated genes for GO enrichment
Verity_astrocyte_prion_markers_up<-subset(Verity_astrocyte_ageing_markers_entrez, subset=avg_log2FC>(0.25)&p_val_adj<0.05) #Added p value filter here
...
VerityPrion_up_GO_OvR = enrichGO(gene = Verity_Prion_DEGs_up, OrgDb = org.Mm.eg.db, readable = T, ont= "BP", pvalueCutoff = 0.05, qvalueCutoff = 0.10)
...
ego4 <- gseGO(geneList     = geneList, #This is the DEG list full list of DEGs
              OrgDb        = org.Mm.eg.db,
              ont          = "ALL",
              minGSSize    = 100,
              maxGSSize    = 500,
...
x <- enrichPathway(gene=Verity_astrocyte_prion_markers_up$entrezgene_id, pvalueCutoff = 0.05, readable=TRUE, organism = "mouse")
head(x)
...
#Visualisation 
viewPathway("Protein-protein interactions at synapses", #specify the pathway of interest
            readable = TRUE, 
