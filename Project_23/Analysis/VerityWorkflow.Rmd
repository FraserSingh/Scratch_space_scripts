...
folder_names <- c("sample1", "sample2", "sample5", "sample6", "sample9","sample11", )
...
for (folder_name in folder_names) {
  ...
  #table(rownames(current_parse) == "") #see if there are any empty gene names
  
  current_parse_interpreted <- CreateSeuratObject(current_parse, meta.data = current_metadata, names.delim = "")
  
  current_parse_interpreted@meta.data$orig.ident <- factor(rep(paste0("verity",sample), nrow(current_parse_interpreted@meta.data)))
...
Verity_total <- merge(sample1_obj, y=c(sample1_obj, sample2_obj, sample5_obj, sample6_obj, sample9_obj,sample11_obj), add.cell.ids=c("sample1", "sample2", "sample5", "sample6", "sample9", "sample11"), project="Verity")

#cleaning up memory
rm("sample1_obj","sample2_obj", "sample5_obj", "sample6_obj", "sample9_obj", "sample11_obj")
```
...
# Homotypic doublet proportion esticurrent_parsee annotations <-
...
# Update metadata column name for doublet inforcurrent_parseion doublet_col_index
