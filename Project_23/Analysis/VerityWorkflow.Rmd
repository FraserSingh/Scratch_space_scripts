...
Ver_singlets_merged@active.assay<-"RNA"

Ver_singlets_merged <- NormalizeData(Ver_singlets_merged, verbose = FALSE)
Ver_singlets_merged <- FindVariableFeatures(Ver_singlets_merged, verbose = FALSE)
Ver_singlets_merged <- ScaleData(Ver_singlets_merged, verbose = FALSE)


# run sctransform
# Ver_singlets_merged <- SCTransform(Ver_singlets_merged, vars.to.regress = "percent.mt", verbose = TRUE, conserve.memory = TRUE)
# 


gc()

Ver_singlets_merged <- RunPCA(Ver_singlets_merged, verbose = TRUE)
ElbowPlot(Ver_singlets_merged)

Ver_singlets_merged <- RunUMAP(Ver_singlets_merged, dims = 1:20, verbose = TRUE)

Ver_singlets_merged <- FindNeighbors(Ver_singlets_merged, dims = 1:20, verbose = TRUE)
Ver_singlets_merged <- FindClusters(Ver_singlets_merged, verbose = TRUE, resolution = 1, random.seed = 42)

DimPlot(Ver_singlets_merged, label = TRUE, raster=FALSE) + NoLegend()+ ggtitle("Verity clustering", subtitle = "NormalizeData, clustering resolution= 1, UMAPdims20")

#label by sample, move the blue colou8r to highlight a different sample https://github.com/satijalab/seurat/issues/2251
dimplotcolours=c(
...
DimPlot(Ver_singlets_merged, label = TRUE, raster=FALSE, group.by = "sample", repel = TRUE)+ ggtitle("Verity clustering", subtitle = "NormalizeData, clustering resolution= 1, UMAPdims20")+ scale_color_manual(values = dimplotcolours)


save(Ver_singlets_merged, file="Rdata/Verity_clustering.RData")
```
