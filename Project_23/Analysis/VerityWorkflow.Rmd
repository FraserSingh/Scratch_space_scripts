...
Ver_singlets_merged <- RunUMAP(Ver_singlets_merged, dims = 1:30, verbose = TRUE)
 
Ver_singlets_merged <- FindNeighbors(Ver_singlets_merged, dims = 1:30, verbose = TRUE)
Ver_singlets_merged <- FindClusters(Ver_singlets_merged, verbose = TRUE, resolution = 1, random.seed = 42)

DimPlot(Ver_singlets_merged, label = TRUE, raster=FALSE) + NoLegend()+ ggtitle("Verity clustering", subtitle = "NormalizeData, clustering resolution= 1 ")
