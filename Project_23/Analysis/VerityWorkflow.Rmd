...
load("./Rdata/Verity_cells.Rdata")
load("./Rdata/Verity_drops.Rdata")
...
#make into soupchannel but make more manual by setting FALSE parameter, this may not be needed
...
#This variable was generated from loading the whole dataset, labelling smaples and splitting by samples to have a list of 6
load("Rdata/Verity_labelled_unfiltered.Rdata")

#Preprocessing to findscale, normalise and find clusters, plot it too if you uncomment the dimplot line
Verity.list <- lapply(Verity.list, function(sample) {
  print(paste0("Sample ",sample@meta.data$sample[1]))
  # Pre-process seurat object with standard seurat workflow. When converse memory is set to TRUE the residual matrix for all genes is never created in full; useful for large data sets, but will take longer to run; this will also set return.only.var.genes to TRUE (https://rdrr.io/github/satijalab/seurat/man/SCTransform.html)
