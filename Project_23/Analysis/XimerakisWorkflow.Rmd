...
```{r}
png("Ximerakis_Preliminary_Data_assessment.png", width=2000)
...
png("Ximerakis_UMIs per cell vs.mitochondrial genes detected.png", width=1000)
...
png("Ximerakis_UMIs_per_cell_vsribosomal_protein_genes_detected.png", width=1000)
...
png("Ximerakis_ribosomal_protein_genes_detected_vsmitochondiral_genes_detected.png", width=1000)
...
png("Ximerakis_UMIs_per_cell_vs_number_of_genes.png", width=1000)
...
png("Ximerakis_Cell_counts_per_sample1.png", width=1500)
metadata_test %>% 
...
png("Ximerakis_Cell_counts_per_sample2.png", width=1000)
metadata_test %>% 
...
png("Ximerakis_UMI_transcripts_per_cell.png", width=1000)
metadata_test %>% 
...
png("Ximerakis_Distribution_of_genes_per_cell1.png", width=1000)
metadata_test %>% 
...
png("Ximerakis_UMIs_vs_genes_detected.png", width=2000)
metadata_test %>% 
...
png("Ximerakis_Distribution_of_mitochondrial_gene_expression_per_cell.png", width=1000)
metadata_test %>% 
...
```{r}
png("Ximerakis_Repeat_mitoratio.png", width=1000)
...
png("Ximerakis_Complexity_of_gene_expression_genes_per_UMI2.png", width=1000)
metadata_test %>%
...
```{r}
# Visualize the number UMIs/transcripts per cell
png("Ximerakis_UMI_transcripts_per_cell2.png", width=1000)
metadata_filtered %>% 
  	ggplot(aes(color=sample, x=nUMI, fill= sample)) + 
  	geom_density(alpha = 0.2) + 
  	scale_x_log10() + 
...
```{r}
# Visualize the distribution of genes detected per cell via histogram
png("Ximerakis_Distribution_of_genes_per_cell3.png", width=1000)
metadata_filtered %>% 
  	ggplot(aes(color=sample, x=nGene, fill= sample)) + 
  	geom_density(alpha = 0.2) + 
  	theme_classic() +
...
# Visualize the distribution of genes detected per cell via boxplot
png("Ximerakis_Distribution_of_genes_per_cell4.png", width=1000)
metadata_filtered %>% 
  	ggplot(aes(x=sample, y=log10(nGene), fill=sample)) + 
  	geom_boxplot() + 
  	theme_classic() +
...
#Some code commented out because it introduced infitine values with a log transform
# Visualize the correlation between genes detected and number of UMIs and determine whether strong presence of cells with low numbers of genes/UMIs
png("Ximerakis_UMIs_vs_genes_detected2.png", width=1000)
metadata_filtered %>% 
  	ggplot(aes(x=nUMI, y=nGene, color=mitoRatio)) + 
  	geom_point() + 
	scale_colour_gradient(low = "gray90", high = "black") +
...
```{r}
# Visualize the distribution of mitochondrial gene expression detected per cell
png("Ximerakis_Distribution_of_mitochondrial_gene_expression_per_cell2.png", width=1000)
metadata_filtered %>% 
  	ggplot(aes(color=sample, x=mitoRatio, fill=sample)) + 
  	geom_density(alpha = 0.2) + 
  	scale_x_log10() + 
...
```{r}
# Visualize the overall complexity of the gene expression by visualizing the genes detected per UMI
png("Ximerakis_Complexity_of_gene_expression_genes_per_UMI_2.png", width=1000)
metadata_filtered %>%
  	ggplot(aes(x=log10GenesPerUMI, color = sample, fill=sample)) +
  	geom_density(alpha = 0.2) +
  	theme_classic() +
...
save(Ximerakis_filtered, file="./Rdata/Ximerakis_predoublet.RData")
```
...
```{r}
# Create .RData object to load at any time
save(Xim_singlets_merged, file="Rdata/Xim_singlets_merged.RData")
