...
Slota_HP_comp <- RunUMAP(Slota_HP_comp, reduction = "pca", dims = 1:30)
...
Slota_HP_comp_astrocytes <- RunUMAP(Slota_HP_comp_astrocytes, reduction = "pca", dims = 1:20)
...
# Upregulated genes
Slota_astrocyte_prion_markers_clean_HP_up<-subset(Slota_astrocyte_prion_markers_clean_HP, subset=avg_log2FC>0.25)

#Create a list of just the genes: 
Slota_HP_DEGs_up = rownames(Slota_astrocyte_prion_markers_clean_HP_up)
...
#GO GSEA
ego3 <- gseGO(geneList     = Slota_DEGs_HPup__entrez$ENTREZID,
              OrgDb        = org.Mm.eg.db,
              ont          = "ALL",
              minGSSize    = 100,
              maxGSSize    = 500,
...
#Finish dimensinoality reduction steps
Slota_CX_comp <- RunUMAP(Slota_CX_comp, reduction = "pca", dims = 1:30)
...
#20 PCs seem to cover the variability
Slota_CX_comp_astrocytes <- RunUMAP(Slota_CX_comp_astrocytes, reduction = "pca", dims = 1:20)
...
# Upregulated genes
Slota_astrocyte_prion_markers_clean_CX_up<-subset(Slota_astrocyte_prion_markers_clean_CX, subset=avg_log2FC>0.25)

#Create a list of just the genes: 
Slota_CX_DEGs_up = rownames(Slota_astrocyte_prion_markers_clean_CX_up)
#Convert this gene SYMBOL list into Entrez ID:
...
ggp = barplot(Slota_CXup_GO_OvR , showCategory=20)
ggp2 = dotplot(Slota_CXup_GO_OvR, showCategory=10)
ggp3 = goplot(Slota_CXup_GO_OvR, showCategory = 10)
ggp4 = cnetplot(Slota_CXup_GO_OvR, categorySize=p.adjust)
...
#GO GSEA
ego3 <- gseGO(geneList     = Slota_DEGs_CXup__entrez$ENTREZID,
              OrgDb        = org.Mm.eg.db,
              ont          = "ALL",
              minGSSize    = 100,
              maxGSSize    = 500,
