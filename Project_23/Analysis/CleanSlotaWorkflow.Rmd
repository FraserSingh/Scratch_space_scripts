...
This workflow loads samples for the control and prion-treated data from Slota group's hippocampal and cortex prion work. The samples are QC assessed, filtered and plotted with clustering by gene expression profiles.
...
Slota_raw_obj_clean<-CreateSeuratObject(counts=rawtable, project="Slota")
```
...
sampleLabs<-names(Slota_raw_obj_clean@active.ident)
...
Slota_raw_obj_clean@meta.data$sample<-sample_detect
Idents(object=Slota_raw_obj_clean)<-"sample"
...
#Slota
sampleLabs_Slota<- names(Slota_raw_obj_clean@active.ident)
...
```{r}
VlnPlot(Slota_raw_obj_clean, features=c("nFeature_RNA","nCount_RNA","percent.mt", "percent.rp"), ncol=4, pt.size = 0.0)#+ggtitle("Preliminary Data assessment", subtitle= "Slota, unfiltered")

# Plot each of these respectively, showing trends in cell health

#show mitochondrial proportion in reads
FeatureScatter(Slota_raw_obj_clean, feature1 = "nCount_RNA", feature2 = "percent.mt", pt.size = 0)+ggtitle("Slota_UMIs per cell vs.mitochondrial genes detected", subtitle="UMIs per cell vs. % mitochondrial genes detected")


#show ribosomal proportion in reads
FeatureScatter(Slota_raw_obj_clean, feature1 = "nCount_RNA", feature2 = "percent.rp")+ggplot2::ggtitle("Slota_UMIs_per_cell_vsribosomal_protein_genes_detected", subtitle = "UMIs per cell vs. % ribosomal protein  genes detected")


#Ribosomal vs mitochondrial 
FeatureScatter(Slota_raw_obj_clean, feature1 = "percent.rp", feature2 = "percent.mt")+ggplot2::ggtitle("Slota_ribosomal_protein_genes_detected_vsmitochondrial_genes_detected",subtitle="% ribosomal protein  genes detected vs. % mitochondrial genes detected")

#Show overall reads and gene counts for unfiltered data.
FeatureScatter(Slota_raw_obj_clean, feature1 = "nCount_RNA", feature2="nFeature_RNA",raster=F, pt.size = 0.5)+ geom_smooth(method="lm")+ ggtitle("Slota_UMIs_per_cell_vs_number_of_genes",subtitle="UMIs per cell vs. number of genes ")
```
...
metadata_filtered<- metadata_test %>%
    filter(nUMI > 0, nGene >150, nUMI <35000, percent.mt <20) #FIXME check upper limit on nUMI
```
...
rm(metadata_test, metadata_filtered,filtered_counts, nonzero,counts, keep_genes)
gc()
```
...
Slota_list_clean <- SplitObject(Slota_filtered_clean, split.by = "sample")
...
Slota_list_clean <- lapply(Slota_list_clean, function(sample) {
  ...
  nExp.poi <- round(0.087 * nrow(sample@meta.data)) #expected doublets based on values from 10X resources
...
VlnPlot(Slo_merged_clean, features = "nFeature_RNA", group.by = 'doublet_finder', pt.size = 0.0)
...
ElbowPlot(Slo_singlets_merged_clean, ndims=30)+ggtitle("Elbowplot of Slota data", subtitle = "NBH and ME7")
...
VlnPlot(Slo_singlets_merged_clean, features = astrocyte_genes)
...
Slo_astrocytes<-subset(Slo_singlets_merged_clean, idents=c('3', '26'))
```
...
ElbowPlot(Slo_astrocytes, ndims=30)+ggtitle("Elbowplot of subset Slota astrocytes", subtitle = "RunPCA npcs=30")

#15 PCs sems ok, so use 20 from now

#20 PCs seem to cover the variability
Slo_astrocytes <- RunUMAP(Slo_astrocytes, reduction = "pca", dims = 1:20)
Slo_astrocytes <- FindNeighbors(Slo_astrocytes, reduction = "pca", dims = 1:20)
Slo_astrocytes <- FindClusters(Slo_astrocytes, resolution = c(0.2,0.3,0.5,0.7,1))

#cyclechange resolution number in group.by to see difference in clusters
DimPlot(Slo_astrocytes, reduction = 'umap',  raster=F, group.by='treatment')+ggtitle("Slota data's astrocytes", subtitle="Labelled by treatment")

#Show the clustering of subset astrocytes
DimPlot(Slo_astrocytes, group.by = 'RNA_snn_res.0.5', reduction = 'umap', repel = TRUE, raster=F)

#Show sample disctribution in clusters of astrocytes
DimPlot(Slo_astrocytes, group.by = 'sample', reduction = 'umap', repel = TRUE, raster=F)

#Show astrocyte markers in astrocyte subset
FeaturePlot(Slo_astrocytes,features=c('Gfap','Cd44'), raster=F)

#set the idents to 0.5
Slo_astrocytes@active.ident<-(Slo_astrocytes@meta.data$RNA_snn_res.0.5)
...
Slota_astrocyte_prion_markers<-FindMarkers(Slo_astrocytes, ident.1 = 'ME7', ident.2 = 'NBH',min.pct = 0.10, verbose = TRUE, test.use = "DESeq2",logfc.threshold = 0.25)

#Sort the list in descending log2FC and ascending pvalues
Slota_astrocyte_prion_markers_clean<-Slota_astrocyte_prion_markers[order(-Slota_astrocyte_prion_markers$avg_log2FC), ]
#Save the file
write.csv(Slota_astrocyte_prion_markers_clean, file = "Slota_astrocyte_prion_markers_clean.csv")

save(Slota_astrocyte_prion_markers_clean, file="Slota_astrocyte_prion_markers_clean.RData")
```
